"""Making Biliniar interpolation on proveded points and known values of a function."""

import click
import numpy as np


def biliniar_interpolation(
    x: float, y: float, known_points: list[tuple[float, float]], values_of_function: list[float]
) -> float:
    """
    Perform bilinear interpolation for given (x,y) coordinates.

    Uses four known points and their corresponding function values.
...
@click.command()
@click.option("--x", default=2.0, type=float, help="Directory of image")
@click.option("--y", default=3.0, type=float, help="Method of interpolation")
@click.option(
    "--known_points",
...
@click.option("--values_of_function", nargs=4, default=[5.0, 6.0, 7.0, 8.0], type=float, help="Axes y resize parameter")
def main(x: float, y: float, known_points: list[tuple[float, float]], values_of_function: list[float]) -> None:
    """Command line interface for bilinear interpolation."""
